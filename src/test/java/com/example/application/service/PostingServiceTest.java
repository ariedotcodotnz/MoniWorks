...
/**
 * Unit tests for PostingService.
 * Tests the core accounting logic for posting transactions to the ledger.
 */
...
class PostingServiceTest {
    ...
    void postTransaction_whenUnbalanced_throwsException() {
        // Arrange
        Transaction transaction = new Transaction(company, Transaction.TransactionType.PAYMENT,
            LocalDate.of(2024, 4, 15));
        transaction.setId(1L);

        // Add only a debit line (unbalanced)
        TransactionLine debitLine = new TransactionLine(bankAccount,
            new BigDecimal("100.00"), TransactionLine.Direction.DEBIT);
        ...
        IllegalStateException exception = assertThrows(IllegalStateException.class,
            () -> postingService.validateForPosting(transaction));
    ...
    void reverseTransaction_createsInvertedEntries() {
        ...
        when(periodRepository.findByCompanyAndDate(any(), any()))
            .thenReturn(Optional.of(openPeriod));
        when(auditService.logEvent(any(), any(), any(), any(), any(), any()))
            .thenReturn(new AuditEvent());
        when(taxCalculationService.createTaxLinesForLedgerEntries(any(), anyList()))
        ...
        TransactionLine reversedBankLine = reversal.getLines().stream()
    ...
    private Transaction createBalancedTransaction() {
        ...
        TransactionLine creditLine = new TransactionLine(bankAccount,
            new BigDecimal("100.00"), TransactionLine.Direction.CREDIT);
        transaction.addLine(creditLine);

        // Debit expense (expense increasing)
        TransactionLine debitLine = new TransactionLine(expenseAccount,
            new BigDecimal("100.00"), TransactionLine.Direction.DEBIT);
        transaction.addLine(debitLine);
