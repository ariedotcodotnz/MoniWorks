...
public interface LedgerEntryRepository extends JpaRepository<LedgerEntry, Long> {
  ...
  @Query(
      "SELECT le FROM LedgerEntry le WHERE le.company = :company "
  ...
  BigDecimal sumDebitsByAccountAsOf(
      @Param("account") Account account, @Param("asOfDate") LocalDate asOfDate);

  @Query(
      "SELECT COALESCE(SUM(le.amountCr), 0) FROM LedgerEntry le "
          + "WHERE le.account = :account AND le.entryDate <= :asOfDate")
  BigDecimal sumCreditsByAccountAsOf(
      @Param("account") Account account, @Param("asOfDate") LocalDate asOfDate);

  @Query(
      "SELECT COALESCE(SUM(le.amountDr) - SUM(le.amountCr), 0) FROM LedgerEntry le "
          + "WHERE le.account = :account AND le.entryDate <= :asOfDate")
  BigDecimal getBalanceByAccountAsOf(
      @Param("account") Account account, @Param("asOfDate") LocalDate asOfDate);
