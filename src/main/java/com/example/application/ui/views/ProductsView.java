...
/**
 * View for managing Products and Services. Features a master-detail layout with: - Left: searchable
 * product list with category filter - Right: product detail view with pricing, tax, and account
 * defaults
 */
...
public class ProductsView extends VerticalLayout {
  ...
  private static final NumberFormat CURRENCY_FORMAT =
      NumberFormat.getCurrencyInstance(new Locale("en", "NZ"));
  ...
  private void showProductDetail(Product product) {
    ...
    addReadOnlyField(
        ...
        product.getPurchaseAccount() != null
            ? product.getPurchaseAccount().getCode()
                + " - "
                + product.getPurchaseAccount().getName()
            : "Not set");
  ...
  private void openProductDialog(Product product) {
    ...
    if (!isNew && product.getDescription() != null)
    ...
    List<String> taxCodes =
        taxCodeService.findActiveByCompany(company).stream().map(tc -> tc.getCode()).toList();
    ...
    if (!isNew && product.getSalesAccount() != null)
      salesAccountCombo.setValue(product.getSalesAccount());
    ...
    if (!isNew && product.getPurchaseAccount() != null)
      purchaseAccountCombo.setValue(product.getPurchaseAccount());
    ...
    if (!isNew && product.getStickyNote() != null)
      stickyNoteField.setValue(product.getStickyNote());
  ...
  private void openImportDialog() {
    ...
    Span instructions =
        new Span(
            "Upload a CSV file with product data. Required columns: code, name. "
    ...
    upload.addSucceededListener(
        event -> {
          try {
            ...
            // Preview the import
            Company company = companyContextService.getCurrentCompany();
            ProductImportService.ImportResult preview =
