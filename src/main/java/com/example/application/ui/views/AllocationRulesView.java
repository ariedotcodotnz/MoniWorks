...
/**
 * View for managing allocation rules used during bank reconciliation. Rules auto-suggest coding
 * (account and tax code) based on transaction descriptions.
 *
 * <p>Features: - List all allocation rules with priority ordering - Create, edit, and delete rules
 * - Enable/disable rules - Test rules against sample descriptions - Priority-based ordering for
 * rule application
 */
...
public class AllocationRulesView extends VerticalLayout {
  ...
  private void configureGrid() {
    ...
    grid.addColumn(AllocationRule::getMatchExpression).setHeader("Match Expression").setFlexGrow(1);

    grid.addColumn(
            rule -> rule.getTargetAccount().getCode() + " - " + rule.getTargetAccount().getName())
  ...
  private void configureDetailForm() {
    ...
    binder.forField(priorityField).bind(AllocationRule::getPriority, AllocationRule::setPriority);

    binder.forField(enabledCheckbox).bind(AllocationRule::isEnabled, AllocationRule::setEnabled);
  ...
  private void saveRule() {
    try {
      ...
      Notification.show("Rule saved successfully", 3000, Notification.Position.BOTTOM_START)
    ...
    Button deleteBtn =
        new Button(
            ...
            e -> {
              ...
              Notification.show("Rule deleted", 3000, Notification.Position.BOTTOM_START)
                  .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
