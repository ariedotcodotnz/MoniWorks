...
/**
 * View for managing Contacts (Customers and Suppliers). Features a master-detail layout with: -
 * Left: searchable contact list - Right: tabbed detail view (General, People, Defaults, Notes)
 */
...
public class ContactsView extends VerticalLayout {
  ...
  private VerticalLayout createDefaultsTab(Contact contact) {
    ...
    addReadOnlyField(
        form,
        "Credit Limit",
        contact.getCreditLimit() != null
            ? "$" + contact.getCreditLimit().toPlainString()
            : "No limit");
  ...
  private VerticalLayout createNotesTab(Contact contact) {
    ...
    notesGrid
        .addColumn(n -> formatTimestamp(n.getCreatedAt()))
        .setHeader("Date")
        .setAutoWidth(true)
        .setFlexGrow(0);

    notesGrid
        .addColumn(n -> n.getCreatedBy() != null ? n.getCreatedBy().getDisplayName() : "")
        .setHeader("By")
        .setAutoWidth(true);

    notesGrid.addColumn(ContactNote::getNoteText).setHeader("Note").setFlexGrow(1);

    notesGrid
        .addColumn(
            n -> n.getFollowUpDate() != null ? n.getFollowUpDate().format(DATE_FORMATTER) : "")
        .setHeader("Follow-up")
        .setAutoWidth(true);
  ...
  private void openContactDialog(Contact contact) {
    ...
    if (!isNew && contact.getAddressLine1() != null)
      addressLine1Field.setValue(contact.getAddressLine1());

    TextField addressLine2Field = new TextField("Address Line 2");
    addressLine2Field.setMaxLength(255);
    if (!isNew && contact.getAddressLine2() != null)
      addressLine2Field.setValue(contact.getAddressLine2());
    ...
    if (!isNew && contact.getPostalCode() != null)
    ...
    if (!isNew && contact.getBankAccountNumber() != null)
      bankAccountField.setValue(contact.getBankAccountNumber());

    TextField bankRoutingField = new TextField("Bank Routing");
    bankRoutingField.setMaxLength(50);
    if (!isNew && contact.getBankRouting() != null)
      bankRoutingField.setValue(contact.getBankRouting());

    // Defaults
    TextField taxOverrideField = new TextField("Tax Override Code");
    taxOverrideField.setMaxLength(10);
    if (!isNew && contact.getTaxOverrideCode() != null)
      taxOverrideField.setValue(contact.getTaxOverrideCode());
    ...
    if (!isNew && contact.getPaymentTerms() != null)
      paymentTermsField.setValue(contact.getPaymentTerms());

    BigDecimalField creditLimitField = new BigDecimalField("Credit Limit");
    if (!isNew && contact.getCreditLimit() != null)
      creditLimitField.setValue(contact.getCreditLimit());
    ...
    form.add(
        defaultAccountCombo, taxOverrideField, paymentTermsField, creditLimitField, activeCheckbox);
  ...
  private void openPersonDialog(Contact contact, ContactPerson person) {
    ...
    saveButton.addClickListener(
        e -> {
          ...
          ContactPerson p =
              isNew ? contactService.createPerson(contact, nameField.getValue().trim()) : person;
  ...
  private void openImportDialog() {
    ...
    Span instructions =
        new Span(
            "Upload a CSV file with contact data. Required columns: code, name. "
    ...
    upload.addSucceededListener(
        event -> {
          try {
            ...
            // Preview the import
            Company company = companyContextService.getCurrentCompany();
            ContactImportService.ImportResult preview =
                contactImportService.previewImport(
        ...
        e -> {
          ...
          try {
            ...
            ContactImportService.ImportResult result =
                contactImportService.importContacts(
