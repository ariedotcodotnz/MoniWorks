...
/**
 * View for bank reconciliation.
 * Two-pane layout: bank feed items on left, matching/create actions on right.
 *
 * Features:
 * - Import bank statements (QIF, OFX, CSV)
 * - View unmatched bank feed items
 * - Match items to existing transactions
 * - Create new transactions from feed items
 * - Ignore items that don't need transactions
 * - Auto-suggest coding based on allocation rules
 */
...
public class BankReconciliationView extends VerticalLayout {
    ...
    public BankReconciliationView(BankImportService bankImportService,
    ...
    private HorizontalLayout createToolbar() {
        ...
        bankAccountSelector.addValueChangeListener(e -> {
    ...
    private void configureFeedItemGrid() {
        ...
        feedItemGrid.addColumn(item -> item.getPostedDate().format(DATE_FORMAT))
            .setHeader("Date")
            .setSortable(true)
            .setAutoWidth(true);

        feedItemGrid.addColumn(this::formatAmount)
            .setHeader("Amount")
            .setSortable(true)
            .setAutoWidth(true);

        feedItemGrid.addColumn(BankFeedItem::getDescription)
            .setHeader("Description")
            .setFlexGrow(1);

        feedItemGrid.addColumn(item -> item.getStatus().name())
            .setHeader("Status")
            .setAutoWidth(true);

        feedItemGrid.addSelectionListener(e -> {
    ...
    private void showEmptyDetailPanel() {
        ...
        placeholder.add(new Paragraph("Click on an item from the list to view details and take action."));
    ...
    private void showFeedItemDetail(BankFeedItem item) {
        ...
        Optional<AllocationRule> suggestedRule = bankImportService.findMatchingRule(
            company, item.getDescription());

        if (suggestedRule.isPresent()) {
            AllocationRule rule = suggestedRule.get();
            Span suggestion = new Span("Suggested: " + rule.getTargetAccount().getCode() +
        ...
        createTransactionBtn.addClickListener(e -> openCreateTransactionDialog(item, suggestedRule.orElse(null)));
    ...
    private void openImportDialog() {
        ...
        upload.addSucceededListener(event -> {
            try {
                ...
                Company company = companyContextService.getCurrentCompany();
                BankStatementImport importRecord = bankImportService.importStatement(
                    company, selectedBankAccount, fileName, content);

                resultArea.setValue("Import successful!\n" +
                    "File: " + fileName + "\n" +
                    "Items imported: " + importRecord.getTotalItems());
                resultArea.setVisible(true);

                loadFeedItems();

                Notification.show("Imported " + importRecord.getTotalItems() + " items",
    ...
    private void openCreateTransactionDialog(BankFeedItem item, AllocationRule suggestedRule) {
        ...
        createBtn.addClickListener(e -> {
            ...
            try {
                ...
                String taxCode = taxCodeCombo.getValue() != null ? taxCodeCombo.getValue().getCode() : null;
                ...
                Notification.show("Transaction created and posted", 3000, Notification.Position.BOTTOM_START)
                    .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
    ...
    private void openMatchDialog(BankFeedItem item) {
        ...
        List<Transaction> candidates = transactionService.findByCompany(company).stream()
            .filter(t -> t.isPosted())
            .filter(t -> t.getLines().stream()
        ...
        candidateGrid.addColumn(t -> t.getTransactionDate().format(DATE_FORMAT))
            .setHeader("Date")
            .setSortable(true)
            .setAutoWidth(true);

        candidateGrid.addColumn(t -> t.getType().name())
            .setHeader("Type")
            .setAutoWidth(true);

        candidateGrid.addColumn(Transaction::getDescription)
            .setHeader("Description")
            .setFlexGrow(1);

        candidateGrid.addColumn(t -> {
