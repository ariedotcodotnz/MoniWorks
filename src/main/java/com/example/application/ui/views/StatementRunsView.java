...
/**
 * View for managing Statement Runs - batch customer statement generation.
 * Allows creating runs with filter criteria, previewing customers, and downloading combined PDFs.
 */
...
public class StatementRunsView extends VerticalLayout {
    ...
    public StatementRunsView(StatementRunService runService,
    ...
    private void configureGrid() {
        ...
        runGrid.addColumn(run -> formatDate(run.getRunDate()))
            .setHeader("Run Date")
            .setSortable(true)
            .setAutoWidth(true);

        runGrid.addColumn(run -> formatDate(run.getAsOfDate()))
            .setHeader("As Of Date")
            .setSortable(true)
            .setAutoWidth(true);

        runGrid.addComponentColumn(this::createStatusBadge)
            .setHeader("Status")
            .setAutoWidth(true);

        runGrid.addColumn(run -> run.getStatementCount() != null ? run.getStatementCount().toString() : "-")
            .setHeader("Statements")
            .setSortable(true)
            .setAutoWidth(true);

        runGrid.addColumn(run -> run.getCreatedBy() != null ? run.getCreatedBy().getDisplayName() : "")
            .setHeader("Created By")
            .setSortable(true)
            .setAutoWidth(true);

        runGrid.addColumn(run -> formatDateTime(run.getCreatedAt()))
            .setHeader("Created At")
            .setSortable(true)
            .setFlexGrow(1);

        runGrid.asSingleSelect().addValueChangeListener(e -> {
    ...
    private void updateDetailPanel() {
        ...
        if (selectedRun.getStatementCount() != null) {
            details.add(createDetailRow("Statements Generated:", selectedRun.getStatementCount().toString()));
        }
    ...
    private void processRun() {
        ...
        try {
            ...
            if (selectedRun.isCompleted()) {
                Notification.show("Statement run completed. Generated " +
                        selectedRun.getStatementCount() + " statements.",
                        5000, Notification.Position.BOTTOM_START)
                    .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
            } else if (selectedRun.isFailed()) {
                Notification.show("Statement run failed: " + selectedRun.getErrorMessage(),
            ...
            Notification.show("Error processing run: " + e.getMessage(),
                    5000, Notification.Position.MIDDLE)
                .addThemeVariants(NotificationVariant.LUMO_ERROR);
        }
    }
    ...
    private void downloadPdf() {
        ...
        attachmentService.findById(selectedRun.getOutputAttachmentId()).ifPresent(attachment -> {
            try {
                ...
                Notification.show("Error downloading PDF: " + e.getMessage(),
                        5000, Notification.Position.MIDDLE)
                    .addThemeVariants(NotificationVariant.LUMO_ERROR);
            }
        });
    ...
    private void openCreateDialog() {
        ...
        minimumDaysOverdueField.setHelperText("Only include customers with invoices overdue >= this many days");

        Checkbox includeZeroBalanceField = new Checkbox("Include Zero Balance Customers");
        includeZeroBalanceField.setValue(false);
        includeZeroBalanceField.setHelperText("Include customers with no outstanding balance (rarely needed)");

        // Customer multi-select
        Company company = companyContextService.getCurrentCompany();
        List<Contact> customers = contactService.findActiveCustomers(company);

        MultiSelectComboBox<Contact> customerSelect = new MultiSelectComboBox<>("Specific Customers (Optional)");
        ...
        previewButton.addClickListener(e -> {
            ...
            List<Contact> preview = runService.previewCustomers(
                    company, asOfDateField.getValue(), criteria);
        ...
        createButton.addClickListener(e -> {
            ...
            try {
                ...
                User currentUser = companyContextService.getCurrentUser();
                StatementRun run = runService.createRun(
                        company, asOfDateField.getValue(), criteria, currentUser);

                Notification.show("Statement run created. Click 'Process Now' to generate statements.",
                        5000, Notification.Position.BOTTOM_START)
                    .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
        ...
        createAndProcessButton.addClickListener(e -> {
            if (asOfDateField.isEmpty()) {
                Notification.show("Please select an As Of Date", 3000, Notification.Position.MIDDLE)
                    .addThemeVariants(NotificationVariant.LUMO_ERROR);
                return;
            ...
            try {
                ...
                User currentUser = companyContextService.getCurrentUser();
                StatementRun run = runService.createAndProcessRun(
                        company, asOfDateField.getValue(), criteria, currentUser);

                if (run.isCompleted()) {
                    Notification.show("Statement run completed. Generated " +
                            run.getStatementCount() + " statements.",
                            5000, Notification.Position.BOTTOM_START)
                        .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
                } else if (run.isFailed()) {
                    Notification.show("Statement run failed: " + run.getErrorMessage(),
                            5000, Notification.Position.MIDDLE)
                        .addThemeVariants(NotificationVariant.LUMO_ERROR);
                }
