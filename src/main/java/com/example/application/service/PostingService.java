...
/**
 * Service responsible for posting transactions to the general ledger. Ensures all accounting rules
 * are enforced: - Debits must equal credits - Transaction date must be in an open period - Posted
 * transactions are immutable - Tax calculations are recorded via TaxLine entities
 */
@Service
@Transactional
public class PostingService {
  ...
  public void validateForPosting(Transaction transaction) {
    ...
    for (TransactionLine line : transaction.getLines()) {
      if (!line.getAccount().isActive()) {
        throw new IllegalStateException("Account is inactive: " + line.getAccount().getCode());
      }
    }
    ...
    Period period =
        periodRepository
            .findByCompanyAndDate(transaction.getCompany(), transaction.getTransactionDate())
            .orElseThrow(
                () ->
  ...
  /**
   * Creates a reversal transaction for a posted transaction. The reversal inverts all debit/credit
   * directions.
   */
  public Transaction reverseTransaction(Transaction original, User actor, String reason) {
    ...
    Transaction reversal =
        new Transaction(original.getCompany(), original.getType(), original.getTransactionDate());
    reversal.setDescription(
        "Reversal: " + original.getDescription() + (reason != null ? " - " + reason : ""));
